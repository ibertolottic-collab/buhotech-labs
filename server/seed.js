...
async function seedToFirestore() {
  ...
  try {
    // Usamos batch de Firestore para insertar eficientemente
    const batch = db.batch();
    const questionsRef = db.collection('questions');
    
    for (const q of questions) {
      const docRef = questionsRef.doc(q.id);
      batch.set(docRef, {
        ...q,
    ...
    await batch.commit();
    console.log("🔥 Firestore database seeded with NEW detailed branching questions!");
  } catch (error) {
